...
from .goemotions_configuration_constant import EMOTION_FILE, DATA_DIR,SENTIMENT_FILE
...
## Required parameters
flags.DEFINE_string("emotion_file",
                    EMOTION_FILE,
...
flags.DEFINE_string("sentiment_file",
                    SENTIMENT_FILE,
...
class DataProcessor(object):
    ...
    def _read_df(cls, input_file, data_type):
        ...
        if input_file.endswith("tsv"):
            ...
            sep = ","
        elif data_type == "test" or data_type =='chat_prediction':
            sep = "\t"
        else:
            print("Filetype not supported for %s" % input_file)
            return None

        if data_type == "test" or data_type =='chat_prediction':
            names = ["text"]
        else:
            names = ["text", "labels"]
    ...
    def _create_examples(self, df, data_type):
        ...
        for (i, row) in df.iterrows():
            ...
            if data_type != "test" and data_type != "chat_prediction":
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        tf.compat.v1.summary.scalar("sentiment_regularization", sent_reg)
        loss += sent_reg
